...
Plugin._compileInSandbox = function( sandbox ) {
    return function( content, filename ) {
        console.log( content, filename, sandbox )
        var self = this,
...
Plugin.prototype.load = function() {
    ...
    self.sandbox = sandbox = vm.createContext({
        ...
        debug: function() {
            var args = [].slice.call( arguments ),
                ...
                sout = util.format.apply( null, [].concat( debugId + ':', args ) )
    ...
    // copy props from the current global
    // XXX: figure out how to properly contextify
    for (k in global) {
